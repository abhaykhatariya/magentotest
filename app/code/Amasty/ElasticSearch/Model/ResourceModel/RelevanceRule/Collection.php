...
class Collection extends \Magento\Framework\Model\ResourceModel\Db\Collection\AbstractCollection
{
    ...
    public function getProductBoostMultipliers($productIds, $date, $websiteId)
    {
        ...
        $select = $this->getConnection()->select()
            ...
            ->where(RelevanceRuleInterface::FROM_DATE . ' = 0 OR ' . RelevanceRuleInterface::FROM_DATE . ' <= ?', $date)
            ->where(RelevanceRuleInterface::TO_DATE . ' = 0 OR ' . RelevanceRuleInterface::TO_DATE . ' >= ?', $date)
            ->group(IndexBuilder::PRODUCT_ID);
        if (count($productIds) < 100) {
            $select->where(IndexBuilder::PRODUCT_ID . ' IN(?)', $productIds);
        } else {
            $select
                ->where(IndexBuilder::PRODUCT_ID . ' >= ?', min($productIds))
                ->where(IndexBuilder::PRODUCT_ID . ' <= ?', max($productIds));
        }
